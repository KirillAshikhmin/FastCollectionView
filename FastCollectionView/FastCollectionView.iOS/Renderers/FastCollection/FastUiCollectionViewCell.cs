...
namespace Binwell.Controls.FastCollectionView.iOS.Renderers.FastCollection {
	public sealed class FastUiCollectionViewCell : UICollectionViewCell {
		...
		FastCollectionCell ViewCell { get; set; }
		...
		public void RecycleCell(object data, FastCollectionTemplateSelector dataTemplate, VisualElement parent) {
			if (ViewCell == null) {
				...
				if (!(dataTemplate is FastCollectionTemplateSelector templateSelector)) throw new NotSupportedException(@"DataTemplate should be FastGridTemplateSelector");
		...
		public FastUiCollectionViewCell(CGRect frame) : base(frame) {
		}
