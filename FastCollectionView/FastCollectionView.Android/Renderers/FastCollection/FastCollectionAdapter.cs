...
namespace Binwell.Controls.FastCollectionView.Droid.Renderers.FastCollection {
	public class FastCollectionAdapter : RecyclerView.Adapter {
		...
		readonly FastCollectionViewRenderer _fastCollectionViewRenderer;

		readonly FastCollectionView.FastCollection.FastCollectionView _gridView;
		readonly PropertyInfo _realParentProperty;

		FastCollectionView.FastCollection.FastCollectionView Element { get; }
		...
		void NewCollection_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e) {
			_fastCollectionViewRenderer.CalculateLayoutRects();
			try {
		...
		public FastCollectionAdapter(IEnumerable items, RecyclerView recyclerView, FastCollectionView.FastCollection.FastCollectionView gridView, DisplayMetrics displayMetrics, FastCollectionViewRenderer fastCollectionViewRenderer) {
			Items = items;
			_recyclerView = recyclerView;
			Element = gridView;
			_displayMetrics = displayMetrics;
			_fastCollectionViewRenderer = fastCollectionViewRenderer;
			_gridView = gridView;
		...
		public override int GetItemViewType(int position) {
			...
			if (Element.ItemTemplateSelector is FastCollectionTemplateSelector selector)
				return selector.GetViewType(item, _gridView);
			return -1;
		}
		...
		public override RecyclerView.ViewHolder OnCreateViewHolder(ViewGroup parent, int viewType) {
			...
			FastCollectionCell gridViewCell = null;
			var cellSize = Size.Zero;

			if (templateSelector.OnSelectTemplateByViewType(viewType, null) is FastCollectionDataTemplate template) {
				gridViewCell = template.CreateContent() as FastCollectionCell;
			...
			_realParentProperty?.SetValue(gridViewCell, _gridView);
		...
		public static View GetViewCell(Context context, FastCollectionCell fastGridCell, ViewGroup parent, Size initialCellSize, EventHandler click, int viewType, FastCollectionView.FastCollection.FastCollectionView element) {
	...
	internal sealed class ViewCellProperties : Java.Lang.Object {
		public FastCollectionCell Cell { get; set; }
