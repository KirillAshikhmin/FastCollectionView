...
{
	public class FastCollectionViewRenderer :
		ViewRenderer<FastCollectionView.FastCollection.FastCollectionView, SwipeRefreshLayout>,
		SwipeRefreshLayout.IOnRefreshListener, IGridViewProvider
	{
		...
		FastCollectionAdapter _adapter;
		int _columns = 1;
		...
		public FastCollectionViewRenderer(Context context) : base(context)
		{
		}
		...
		protected override void OnConfigurationChanged(Configuration newConfig)
		{
			...
			if (newConfig.Orientation != _orientation)
				OnElementChanged(
					new ElementChangedEventArgs<FastCollectionView.FastCollection.FastCollectionView>(Element,
						Element));
		}

		protected override void OnElementChanged(
			ElementChangedEventArgs<FastCollectionView.FastCollection.FastCollectionView> e)
		{
		...
		protected override void OnElementPropertyChanged(object sender, PropertyChangedEventArgs e)
		{
			...
			if (e.PropertyName == FastCollectionView.FastCollection.FastCollectionView.ItemsSourceProperty.PropertyName)
			{
				...
				_recyclerView?.GetLayoutManager()?.ScrollToPosition(0);
			         ...
			         FastCollectionView.FastCollection.FastCollectionView.ItemTemplateSelectorProperty.PropertyName)
			{
			...
			else if (e.PropertyName == FastCollectionView.FastCollection.FastCollectionView.IsScrollEnabledProperty
				         .PropertyName)
			{
				Device.BeginInvokeOnMainThread(() =>
				{
					if (_recyclerView != null && Element != null) _recyclerView.Enabled = Element.IsScrollEnabled;
			         ...
			         FastCollectionView.FastCollection.FastCollectionView.IsRefreshingProperty.PropertyName)
			{
				if (_refresh != null && Element != null)
					_refresh.Refreshing = Element.IsRefreshing;
			}
			else if (e.PropertyName == FastCollectionView.FastCollection.FastCollectionView
			...
			else if (e.PropertyName == FastCollectionView.FastCollection.FastCollectionView.RefreshTopOffsetProperty
				         .PropertyName)
			{
				if (_refresh != null && Element != null)
				{
		...
		void CreateRecyclerView()
		{
			...
			_adapter = new FastCollectionAdapter(Element.ItemsSource, _recyclerView, Element, Resources.DisplayMetrics,
				this);
		...
		Size GetSizeByItem(FastCollectionTemplateSelector templateSelector, object item)
		{
			var cellSize = Size.Zero;
			if (templateSelector?.OnSelectTemplate(item) is FastCollectionDataTemplate template)
			{
